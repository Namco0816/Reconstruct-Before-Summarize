...
class Trainer:
    ...
    def evaluation_loop(
        ...
        labels_host = None
        inputs_host = None
        max_token_loss_index_host = None
        ...
        for step, inputs in enumerate(dataloader):
            ...
            # Prediction step
            loss, logits, labels, max_token_loss_index = self.prediction_step(model, inputs, prediction_loss_only, ignore_keys=ignore_keys)
            ...
            if max_token_loss_index is not None:
                max_token_loss_index = self._pad_across_processes(max_token_loss_index)
                max_token_loss_index = self._nested_gather(max_token_loss_index)
                max_token_loss_index_host = (
                        max_token_loss_index
                        if max_token_loss_index_host is None
                        else nested_concat(
                            max_token_loss_index_host,
                            max_token_loss_index,
                            padding_index = -100
            ...
            if args.eval_accumulation_steps is not None and (step + 1) % args.eval_accumulation_steps == 0:
                ...
                if max_token_loss_index_host is not None:
                    max_token_loss_index = nested_numpify(max_token_loss_index_host)
                    all_max_token_loss_index = (
                            max_token_loss_index
                            if all_max_token_loss_index is None
                            else nested_concat(
                                all_max_token_loss_index,
                                max_token_loss_index,
                                padding_index = -100
        ...
        if all_max_token_loss_index is not None:
            all_max_token_loss_index = nested_truncate(all_max_token_loss_index, num_samples)


        if all_max_token_loss_index is not None:
            checkpoint_folder = f"mlm-save_index-{PREFIX_CHECKPOINT_DIR}-{self.state.global_step}"
            max_token_index_path = os.path.join(self.args.output_dir, checkpoint_folder)
            if not os.path.exists(max_token_index_path) and self.is_local_process_zero():
                os.mkdir(max_token_index_path)
            max_token_index_path = os.path.join(max_token_index_path, "token_index.pt")
            if self.is_local_process_zero():
                torch.save(all_max_token_loss_index, max_token_index_path)

        # Metrics!
        if self.compute_metrics is not None and all_preds is not None and all_labels is not None:
            if args.include_inputs_for_metrics:
                metrics = self.compute_metrics(
                    EvalPrediction(predictions=all_preds, label_ids=all_labels, inputs=all_inputs), step = self.state.global_step
                )
            else:
                metrics = self.compute_metrics(EvalPrediction(predictions=all_preds, label_ids=all_labels), step = self.state.global_step)
        else:
            metrics = {}
    ...
    def prediction_step(
            ...
            if is_sagemaker_mp_enabled():
                ...
                if has_labels:
                    if isinstance(raw_outputs, dict):
                        ...
                        logits_mb = tuple(v for k, v in raw_outputs.items() if k not in ignore_keys + ["loss"]+ ['max_token_loss_index'])
                        max_token_loss_index_mb = raw_outputs['max_token_loss_index']
                    else:
                        loss_mb = raw_outputs[0]
                        logits_mb = raw_outputs[1:]
                        max_token_loss_index_mb = raw_outputs[-1]

                    loss = loss_mb.reduce_mean().detach().cpu()
                    logits = smp_nested_concat(logits_mb)
                    max_token_loss_index = smp_nested_concat(max_token_loss_index_mb)
                else:
                    loss = None
                    max_token_loss_index = None
                    if isinstance(raw_outputs, dict):
                ...
                if has_labels:
                    ...
                    if isinstance(outputs, dict):
                        logits = tuple(v for k, v in outputs.items() if k not in ignore_keys + ["loss"] + ['max_token_loss_index'])
                        max_token_loss_index = outputs['max_token_loss_index']
                    else:
                        logits = outputs[1:]
                        max_token_loss_index = outputs[-1]
                else:
                    loss = None
                    max_token_loss_index = None
                    with self.compute_loss_context_manager():
        ...
        if max_token_loss_index is not None:
            max_token_loss_index = nested_detach(max_token_loss_index)
        if prediction_loss_only:
            return (loss, None, None, max_token_loss_index)

        logits = nested_detach(logits)
        if len(logits) == 1:
            logits = logits[0]

        return (loss, logits, labels, max_token_loss_index)
    ...
    def prediction_loop(
        ...
        max_token_loss_index_host: Union[torch.Tensor, List[torch.Tensor]] = None

        world_size = max(1, args.world_size)

        eval_losses_gatherer = DistributedTensorGatherer(world_size, num_examples, make_multiple_of=batch_size)
        eval_token_loss_index_gatherer = DistributedTensorGatherer(world_size, num_examples, make_multiple_of = batch_size)
        ...
        for step, inputs in enumerate(dataloader):
            ...
            if args.eval_accumulation_steps is not None and (step + 1) % args.eval_accumulation_steps == 0:
                ...
                eval_token_loss_index_gatherer.add_arrays(self._gather_and_numpify(max_token_loss_index_host, "eval_max_token_loss_indices"))
                if not prediction_loss_only:
                ...
                # Set back to None to begin a new accumulation
                losses_host, preds_host, labels_host, inputs_host, max_token_loss_index_host = None, None, None, None, None
        ...
        eval_max_token_loss_index = eval_token_loss_index_gatherer.finalize()
        preds = preds_gatherer.finalize() if not prediction_loss_only else None
        label_ids = labels_gatherer.finalize() if not prediction_loss_only else None
        inputs_ids = inputs_gatherer.finalize() if not prediction_loss_only else None

        if eval_max_token_loss_index is not None:
            checkpoint_folder = f"{PREFIX_CHECKPOINT_DIR}-{self.state.global_step}"
            max_token_index_path = os.path.join(self.args.output_dir, checkpoint_folder)
            if not os.path.exists(max_token_index_path):
                os.mkdir(max_token_index_path)
            max_token_index_path = os.path.join(max_token_index_path, "token_index.pt")
            if self.is_local_process_zero():
                torch.save(eval_max_token_loss_index, max_token_index_path)
