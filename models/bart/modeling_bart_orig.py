...
class BartSoftTruncation(BartPretrainedModel):
    base_model_prefix = "model"
    _keys_to_ignore_on_load_missing = [r"final_logits_bias", r"lm_head.weight"]
    ...
    def forward(
        ...
        past_key_values: Optional[List[torch.FloatTensor]] = None,
        inputs_embeds: Optional[torch.FloatTensor] = None,
        anchor_merge_index: Optional[torch.Tensor] = None,
        decoder_inputs_embeds: Optional[torch.FloatTensor] = None,
        labels: Optional[torch.LongTensor] = None,
        use_cache: Optional[bool] = None,
        output_attentions: Optional[bool] = None,
        ...
        if anchor_merge_index is not None:
            max_src_length = torch.max(anchor_merge_index) + 1
            src_embeds = self.model.get_encoder().embed_tokens(input_ids)
            anchor_merge_index = anchor_merge_index.unsqueeze(2)
            anchor_merge_index = anchor_merge_index.expand(-1, -1, src_embeds.shape[-1])
            inputs_embeds = torch.zeros(
                src_embeds.shape[0], 
                max_src_length, 
                src_embeds.shape[-1], 
                requires_grad = True, 
                dtype = src_embeds.dtype,
            ...
            inputs_embeds = inputs_embeds[:, :1024, :]
