...
def load_stopwords(path):
    ...
    for line in codecs.open(path, 'r', 'utf-8'):
        if not re.search('^#', line) and len(line.strip()) > 0:
            stopwords.add(line.strip())  # lowercase
    print(stopwords)
...
def read_ami_icsi(path, filler_words):
    ...
    for tmp in zip(asr_output['role'].tolist(), asr_output['utt'].tolist()):
        role, utt = tmp
        for ch in ['{vocalsound}', '{gap}', '{差距}', '{disfmarker}', '{comment}', '{pause}', '@reject@', '{间隙}', '{人声}', '{缺口}', ]:
            utt = re.sub(ch, '', utt)
...
if domain == 'meeting':
    ...
    elif dataset_id == 'ami_new':
        ids = []
        raw_ids = make_meeting_list.ami_zh_meeting_list
        for id in raw_ids:
            sub_ids = glob.glob(f'resources/data/meeting/ami_new/{id}_*.da.zh')
            sub_ids = [i.split("/")[-1].strip(".da.zh") for i in sub_ids]
            ids += sub_ids
...
for id in ids:
    ...
    for utterance_indexed in utterances_indexed:
        ...
        if len(utt_cleaned) >= min_words:
            cleaned_text = "".join(utt_cleaned)
            if role != current_role:
                utt = role_dict[role] + ": " + utt
                cleaned_text = role_dict[role] + ": " + cleaned_text
                current_role = role
