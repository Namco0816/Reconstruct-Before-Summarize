...
ami = datasets.load_from_disk(f"dataset/{dataset_id}_with_attn")
train = ami['train']
val = ami['validation']
test = ami['test']
channel_ks = [0.025, 0.02, 0.015, 0.01, 0.008, 0.006]
avg_ks = [0.2, 0.16, 0.12, 0.08, 0.064, 0.048]
for channel_k, avg_k in zip(channel_ks, avg_ks):
    ...
    ami.save_to_disk(f"dataset/{dataset_id}_with_segments_c{channel_k}_a{avg_k}_attn")
